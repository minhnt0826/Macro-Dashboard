rescale_range <- function(x, left, right) {
  x_rescaled = (((x - min(x)) / (max(x) - min(x))) * (right - left)) + left
}
...
emp_to_pop_prime_ratio = fred_request_data("LNS12300060") %>%
  mutate(emp_to_pop_prime_ratio = value)
...
job_leavers_percent = fred_request_data("LNS13023706") %>%
  mutate(job_leavers = SMA(value,2))
...
job_losers_unrate = fred_request_data("U2RATE") %>%
  mutate(job_losers = value)

jolt_hires = fred_request_data("JTS1000HIL") %>%
  mutate(hires = value)
...
construction_emp = fred_request_data("USCONS") %>%
  mutate(construction = value) %>%
  mutate(construction_m_change = (value / lag(value, m_chng) - 1) * 100)
...
challenger_job_cuts = read.csv("data/Challenger/challenger_cuts.csv")%>%
  mutate(date = as.Date(date)) %>%
  mutate(challenger_job_cuts = value)
# 
...
# 
jolts = fred_request_data("JTS1000JOL") %>%
  mutate(jolts = value) 
...
dataset2 = temp_help %>%
  ...
  left_join(emp_to_pop_prime_ratio, by = "date") %>%
  ...
  left_join(job_losers_unrate, by = "date") %>%
...
dataset_rotated = dataset2 %>%
  ...
  rename("activity" = "RC1") %>%
  rename("momentum" = "RC2") %>%
  mutate(activity_rescaled = rescale_range(x = activity, left = 1, right = 1550)) %>%
...
# 
# dataset_unrotated = dataset2 %>% dplyr::select("date") %>%
...
plot_ly(dataset_rotated, x=~date, y=~chng_6m, type = "scatter", mode = "lines")


plot_ly(dataset_rotated, x=~date, y=~activity_rescaled, type = "scatter", mode = "lines")

# plot_ly(dataset_rotated, x=~date, y=~momentum, type = "scatter", mode = "lines")

# plot_ly(dataset_unrotated, x=~date, y=~activity, type = "scatter", mode = "lines")
